...
export default function BlogSection() {
  ...
  const visibleCount = isMobile ? 1 : 3;
  const maxIndex = Math.max(0, publishedPosts.length - visibleCount);
  ...
  const handleTouchEnd = () => {
    if (!touchStart || !touchEnd) return;
    const distance = touchStart - touchEnd;
    if (distance > 50 && currentIndex < maxIndex) goToNext();
    if (distance < -50 && currentIndex > 0) goToPrev();
    setTouchStart(null);
    setTouchEnd(null);
  };
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 py-12 sm:py-20 overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-8 sm:mb-12 gap-4">
        ...
        {/* Navigation arrows — only show if posts available */}
        {publishedPosts.length > visibleCount && (
          <div className="hidden sm:flex items-center gap-3">
            <button
              onClick={goToPrev}
              disabled={currentIndex === 0}
      ...
      {/* Body — shows state-based content below the title */}
      {loading ? (
        ...
        <div className="text-center text-gray-400 py-12">
          Posts unavailable right now. Check back soon!
        </div>
        ...
        <>
          ...
          <div className="relative overflow-hidden">
            ...
            >
              {publishedPosts.map((post) => (
                <div
                  key={post.id}
                  className={`w-full flex-shrink-0 px-2 sm:px-4 ${isMobile ? "w-full" : "md:w-1/3"}`}
                >
                  <BlogCard post={post} />
                </div>
              ))}
          ...
          {publishedPosts.length > visibleCount && (
            <div className="flex justify-center gap-1.5 sm:gap-2 mt-8 sm:mt-12">
              {Array.from({
                length: publishedPosts.length - visibleCount + 1,
              }).map((_, index) => (
                <button
                  key={index}
                  onClick={() => goToSlide(index)}
                  className={`h-1.5 sm:h-2 rounded-full transition-all duration-300 ease-out ${index === currentIndex ? "w-6 sm:w-8 bg-gray-900" : "w-1.5 sm:w-2 bg-gray-300 hover:bg-gray-400"}`}
                  aria-label={`Go to slide ${index + 1}`}
                />
              ))}
            </div>
          ...
          {/* Mobile swipe hint */}
          {isMobile && publishedPosts.length > 1 && (
            <p className="text-center text-xs text-gray-400 mt-4 sm:hidden">
              ← Swipe to see more posts →
            </p>
          )}
