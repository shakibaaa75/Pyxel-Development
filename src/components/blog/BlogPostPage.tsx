...
//                 >
//                   <img
//                     src="https://images.unsplash.com/photo-1581094794329-c8112c4e5190?w=400&h=400&fit=crop"
...
//                 >
//                   <img
//                     src="https://images.unsplash.com/photo-1545259741-2ea3ebf61fa3?w=400&h=400&fit=crop"
...
const getSiteId = () => {
  return localStorage.getItem("cms_siteId") || "construction-site";
};
...
export default function BlogPostPage() {
  ...
  const { post, loading, error } = useBlogPost(siteId, slug);
  const { posts: allPosts } = useBlogPosts(siteId);
  const [instagramPosts, setInstagramPosts] = useState<any[]>([]);
  const [searchQuery, setSearchQuery] = useState("");
  const [showSearchResults, setShowSearchResults] = useState(false);

  // Filter only published posts for search and sidebar
  const publishedPosts = allPosts.filter((p) => p.published);

  // Search results
  const searchResults = useMemo(() => {
    if (!searchQuery.trim() || !publishedPosts) return [];
    const query = searchQuery.toLowerCase().trim();
  ...
  useEffect(() => {
    if (post) {
      document.title = `${post.title} - Pyxel Construction Blog`;
  ...
  const readingTime =
    post.readTime || parseInt(calculateReadingTime(post.content || ""));
  ...
  const topPosts = publishedPosts.filter((p) => p.id !== post.id).slice(0, 5);
                  ...
                  <li key={topPost.id}>
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <span className="text-xs text-gray-400">
                          {new Date(topPost.createdAt).toLocaleDateString()}
                        </span>
                      </div>
                    </Link>
                  </li>
